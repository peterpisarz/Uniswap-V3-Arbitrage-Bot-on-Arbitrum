...
const main = async () => {
  ...
  const inputX = await iterateAmountIn([uRouter, sRouter], 5)
  ...
  uPair.on('Swap', async () => {
    if (!isExecuting) {
      ...
      // const input = await iterateAmountIn(routerPath, priceDifference)
...
const iterateAmountIn = async (_routerPath, _priceDifference) => {
  ...
  const priceDiff = BigInt(Number((_priceDifference * 10) + 1000).toFixed(0))

  for (let i = 0; i < 100; i++) {
    b = a * 997n * buyReserves[0] / ((buyReserves[1] * 1000n) + (a * 997n));

    c = b * 997n * sellReserves[1] / ((sellReserves[0] * 1000n) + (b * 997n)) * priceDiff / 1000n;

    profitable = c-a-(24000000000000000n) // output - input - gas
