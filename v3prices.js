...
const { getTokenAndContract, getPairContract, getReserves, calculatePrice, calculatePriceInv, entropy, simulate, getV3Price, getQuote } = require('./helpers/helpers')
...
const main = async () => {
  ...
  const quoteSwap = async () => {
    ...
    // 
    if (best>0) {
      console.log(`Potential arbitrage profit of ${ethers.formatEther(best)} available using ${ethers.formatEther(bestAmount)}. Proceeding with swap!\n`)
      profitable = true
